...
export const POST = withSecurity(SECURITY_PRESETS.AI_GENERATION, async (request, ctx) => {
  ...
  if (body.mode === 'fast') {
    return NextResponse.json({ error: 'fast_mode_unsupported' }, { status: 400 });
  }
  try {
    const result = await generateTopics({
      ...
      excludeTopicKeys: body.excludeTopicKeys as string[] | undefined,
      includeCandidatePool: (body.includeCandidatePool as boolean | undefined) ?? true,
      mode: 'smart',
